...
def get_category_id(category_name: str, client_id: str, access_token: str) -> str | None:
...
def get_top_clips(
    ...
    while len(clips) < amount:
        params: dict = {
            "game_id": category_id,
            "first": min(100, amount - len(clips)),
...
def get_creator_id(username: str, client_id: str, access_token: str) -> str | None:
    headers = {"Authorization": f"Bearer {access_token}", "Client-Id": client_id}
    try:
        response = requests.get(
...
def get_top_clips_creator(
    ...
    while len(clips) < amount:
        ...
        try:
            ...
            print(f"Error getting creator clips: {e}")
            break

    return clips


def download_video(clip_info: dict, video_number: int) -> None:
    url = clip_info["url"]
    download_dir = "./build/videos"
...
def download_thumbnail(thumbnail_url: str, video_number: int) -> bool:
...
def download_clip(clip_info: dict, video_number: int) -> bool:
    thumbnail_success = download_thumbnail(clip_info.get("thumbnail_url", ""), video_number)
    download_video(clip_info, video_number)
    return thumbnail_success


def conseguirVids(choice: str, language: str) -> list[dict]:
    ...
    if choice == "1":
        ...
        amount = int(input("Enter amount of videos: "))
        ...
        for i, clip in enumerate(all_clips[:amount], 1):
            download_clip(clip, i)
            time.sleep(1)

        return [
            {"name": clip["broadcaster_name"], "video": f"{idx}.mp4", "thumbnail": f"{idx}.jpg", "x": "", "y": ""}
